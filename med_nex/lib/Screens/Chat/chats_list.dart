    ...
    if(allChats != null){
      for(var chat in allChats.docs){
        if((chat.get('patientId') == widget.currUser.uid || chat.get('doctorId') == widget.currUser.uid) && chat.get('status') == "active"){
          chats.add(Chat(chat.id, chat.get('doctorId'), chat.get('doctorName'), chat.get('patientId'), chat.get('patientName'), chat.get('request'), chat.get('status'), chat.get('lastMessage'), chat.get('lastMessageTime')));
        }
      }
    }
